...
/** Laravel pagination labels contain HTML entities (&laquo;/&raquo;); render as plain text. */
    ...
    <PublicLayout>
        <div class="mx-auto max-w-6xl px-6 py-16">
            ...
            <p class="mt-3 max-w-xl text-lg text-[var(--ni-gray-light)]">
                Notes from real projects: web development, accessibility and
                building products for small businesses.
            </p>

            <p
                v-if="posts.data.length === 0"
                class="mt-12 text-[var(--ni-gray-light)]"
            >
            ...
            <ul v-else class="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                <li v-for="post in posts.data" :key="post.slug">
                    <article
                        class="flex h-full flex-col overflow-hidden rounded-xl border border-[var(--ni-gray-lightest)]"
                    >
                        ...
                        <div class="flex flex-1 flex-col p-6">
                            ...
                            <p
                                class="mt-3 flex-1 leading-relaxed text-[var(--ni-gray-light)]"
                            >
                                {{ post.excerpt }}
                            </p>
                ...
                <ul class="flex flex-wrap gap-1">
                    <li v-for="link in posts.links" :key="link.label">
                        <Link
                            ...
                            :class="
                                link.active
                                    ? 'bg-[var(--ni-gray)] text-white'
                                    : 'text-[var(--ni-gray)] hover:bg-[var(--ni-gray-lightest)]'
                            "
                        >
